...
class Extractor():
    """
    Reads extract.nc files from EPIC simulation and converts into a dictionary of variables
    ...
    def getextractmatch(self):
        """
        Loops through the folder and finds extract files. Called by default, unles getextractmatch=False
        ...
        #files = [f for f in os.listdir(self.imgfolder) if os.path.isfile(os.path.join(self.imgfolder,f))]
    ...
    def add_restarts(self, resfolder, start=-1):
        ...
        #files = [f for f in os.listdir(resfolder) if os.path.isfile(os.path.join(resfolder,f))]
    ...
    def setup_extract(self, auto_vars = True, get_time=True, \
            ...
            species = [], phase=['liquid','rain','vapor','snow','solid']):
        '''
            Initialize the bookkeeping and get basic properties of the outputs
            (e.g., thermo variables, grid sizes, extents etc.)
    ...
    def setup_time(self):
        ...
        for i, ti in enumerate(self.iarr):
            #fname = self.imgfolder+"/extract%d.nc"%(ti)
            fname = self.iarr[i]
            dset = ncdf.Dataset(fname,'r')
    ...
    def set_shape_factors(self):
        ...
        for j in range(self.gridnj+1):
            ...
            rln_pv = self.grid_re/np.sqrt( 1. + (self.grid_rp/self.grid_re*np.tan(lat_pv))**2.)
            rlt_pv = rln_pv/\
                (np.cos(lat_pv)*(\
                    np.sin(lat_pv)**2. + (self.grid_re/self.grid_rp*np.cos(lat_pv))**2.))

            self.m_pv[:,j] = 1./(rln_pv*self.dln)
            self.n_pv[:,j] = 1./(rlt_pv*self.dlt)
            
            rln_h = self.grid_re/np.sqrt( 1. + (self.grid_rp/self.grid_re*np.tan(lat_h))**2.)
            rlt_h = rln_h/\
                (np.cos(lat_h)*(\
                    np.sin(lat_h)**2. + (self.grid_re/self.grid_rp*np.cos(lat_h))**2.))
    ...
    def get_vars(self,i):
        ...
        for j, var in enumerate(self.varlist):
            #vars[var] = np.zeros((self.gridnk+1, self.gridnj+1, self.gridni))
    ...
    def get_ertel_pv(self, var):
        ...
        for ii in range(self.gridni):
            ...
            theta_av[:,1:,ii] = ((theta[:,:-1,ii-1] + theta[:,:-1,ii])*(mm1*nm1) \
                        + (theta[:,1:,ii] + theta[:,1:,ii-1])*(mp1*np1))*mn_pv
...
def fit_ellipse(v):
    ''' 
        ...
        Used to fit ellipses to a set of 2-D points. Used in Hadland et al. (2020) and
        Sankar et al. (2021)
    '''
