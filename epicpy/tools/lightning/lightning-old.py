...
def dEtotal(
    ...
    if qH2Oliquidcloud > 0:
        Ns[1, H2Oliquidbin] = 53800000.0 * ((rho_dry * qH2Oliquidcloud) ** 0.75)
    ...
    if qNH3liquidcloud > 0:
        Ns[4, NH3liquidbin] = 53800000.0 * ((rho_dry * qNH3liquidcloud) ** 0.75)
    ...
    for s in range(len(binbounds) - 1):
        # Liquid
        velocities[1, s] = 2376.36 * ((r0s[s]) ** 0.7308) * ((100000.0 / P) ** 0.3568)
        velocities[4, s] = 2479.0 * ((r0s[s]) ** 0.76217) * ((100000.0 / P) ** 0.33)
        # Snow
        velocities[2, s] = 42.83 * ((r0s[s]) ** 0.5271) * ((100000.0 / P) ** 0.3043)
        velocities[5, s] = 41.74 * ((r0s[s]) ** 0.5386) * ((100000.0 / P) ** 0.3008)
        # Ice
        velocities[0, s] = 535.41 * ((r0s[s]) ** 0.8746) * ((100000.0 / P) ** 0.1264)
        velocities[3, s] = 495.22 * ((r0s[s]) ** 0.8807) * ((100000.0 / P) ** 0.1248)
    ...
    # Now, calculate dE/dt
    dEdt, invtc = dEdt6(Ns, binbounds, velocities, mus, Qcoeffs, Ebreakdown)
    return dEdt, invtc
