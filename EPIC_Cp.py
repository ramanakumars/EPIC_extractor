...
class Planet():
    ...
    def thermo_setup(self):
        ...
        tvector  = np.zeros(MDIM_THERMO)
        thvector = np.zeros(MDIM_THERMO)
        aa       = np.zeros(MDIM_THERMO)
        ...
        c1    = np.log(K_B*t0/p0*(2.*np.pi*\
                (M_PROTON/H_PLANCK)*(K_B*t0/H_PLANCK))**1.5)
        c2    = np.log(9.)
        p     = p0
        theta = 87.567
        ...
        for i in range(MDIM_THERMO):
            ...
            if(temperature < 10.):
                ...
                so = -np.log(p/p0)+2.5*np.log(temperature)\
                      +1.5*np.log(2.0)+c1+(ho+2.*theta)/temperature+a[5]
